...
public class VMSyncClient {
    public static void broadcastCommit(String targetIp, int targetPort, String sqlCommand) {
        ...
        try (Socket socket = new Socket(targetIp, targetPort);
            ...
            // Event logging
            EventLogger.getInstance().logEvent("[NETWORK] Broadcasted sync to " + targetIp,
                    "REPLICA_SYNC|" + sqlCommand, VMID.resolveMachineIdentity());
            System.out.println("[NETWORK] Broadcasted sync to " + targetIp);

        } catch (Exception e) {
            EventLogger.getInstance().logEvent("[NETWORK] Failed to reach replica at" + targetIp, e.getMessage(),
                    VMID.resolveMachineIdentity());
            System.err.println("[NETWORK] Failed to reach replica at " + targetIp + ": " + e.getMessage());
        }
    }
    
    public static void broadcastCommitENV(String sqlCommand) {
        ConfigManager config = ConfigManager.getInstance();
        String targetIp = config.getProperty("targetIp", "127.0.0.1");
        int targetPort = Integer.parseInt(config.getProperty("targetPort", "9090"));

        broadcastCommit(targetIp, targetPort, sqlCommand);
    }
}
